...
func TestCallTool_ReconnectsWhenSessionClientIsClosing(t *testing.T) {
	originalConnectServerFunc := connectServerFunc
	t.Cleanup(func() {
		connectServerFunc = originalConnectServerFunc
	})
	...
	staleConn, staleTransport, err := newScriptedServerConnection(
		"session-1",
		nil,
		fmt.Errorf(`connection closed: calling "tools/call": client is closing: sending "tools/call":`),
	)
	if err != nil {
		t.Fatalf("newScriptedServerConnection(stale) error = %v", err)
	}
	freshConn, freshTransport, err := newScriptedServerConnection(
		...
		&sdkmcp.CallToolResult{
			Content: []sdkmcp.Content{
				&sdkmcp.TextContent{Text: "reconnected after client close"},
			},
		},
		nil,
	)
	...
	if text.Text != "reconnected after client close" {
		t.Fatalf("CallTool() text = %q, want %q", text.Text, "reconnected after client close")
	}
	if connectCalls != 1 {
		t.Fatalf("connectCalls = %d, want 1", connectCalls)
	}
