...
func TestDelegateToolNotRegistered_SingleAgent(t *testing.T) {
	// Single-agent setup: delegate_to_agent should not be registered
	...
	if _, has := agent.Tools.Get("delegate_to_agent"); has {
		t.Error("delegate_to_agent tool should not be registered in single-agent setup")
	}
}
...
func TestDelegateToolRegistered_MultiAgent(t *testing.T) {
	...
	cfg.Agents.List = []config.AgentConfig{
		{ID: "alpha", Workspace: filepath.Join(tmpDir, "alpha")},
		{ID: "beta", Workspace: filepath.Join(tmpDir, "beta")},
	}

	al := NewAgentLoop(cfg, bus.NewMessageBus(), &mockProvider{})

	// Both agents should have the Zehn delegate_to_agent tool when enabled.
	for _, id := range []string{"alpha", "beta"} {
		...
		if _, has := agent.Tools.Get("delegate_to_agent"); !has {
			t.Errorf("agent %q should have delegate_to_agent tool in multi-agent setup", id)
		}
	}
}
